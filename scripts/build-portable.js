 ...
 *   └─ ...(Electron 运行时文件)
 *
 * 为什么要在 electron-builder 之后再做一步「瘦身」：
...
const BUILD_OUT_REL = path.relative(ROOT, path.join(OUT_ROOT, '.tmp-build')).replace(/\\/g, '/')
const UNPACKED = path.join(OUT_ROOT, '.tmp-build', 'win-unpacked')
...
async function main() {
  step('1/5 构建渲染层 (vite build)')
  run(NODE_BIN, [path.join('node_modules', 'vite', 'bin', 'vite.js'), 'build'])

  step('2/5 打包 Electron 应用 (electron-builder --dir)')
  remove(path.join(OUT_ROOT, '.tmp-build'))
  run(NODE_BIN, [
    path.join('node_modules', 'electron-builder', 'cli.js'),
    '--win', '--x64',
    '--config.directories.output=' + BUILD_OUT_REL
  ])
  ...
  remove(path.join(OUT_ROOT, '.tmp-build'))                        // 清理临时构建目录（失败不致命）
