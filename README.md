...
| 方式 | 说明 |
|---|---|
| **在线刮削** | 输入番号，自动从 JavBus / JavDb 抓取封面、标题、演员、标签、发行日期等完整元数据 |
| **手动添加** | 逐字段填写影片信息，完全自定义 |
| **扫描目录** | 选择本地文件夹，递归扫描所有视频文件，自动从文件名提取番号并批量导入 |
| **单个视频** | 选择单个视频文件，自动识别番号后进入编辑流程 |
| **NFO 导入** | 兼容 Kodi / Jellyfin / Emby 的 NFO（XML）元数据文件，一键迁移现有片库 |
